...
import { duyuruSil, duyuruYayindaDegistir } from "./actions";
...
export function YayinAnahtari({
  ...
  return (
    <button
      type="button"
      disabled={bekliyor}
      onClick={() => baslat(() => duyuruYayindaDegistir(id, !deger))}
      aria-pressed={deger}
...
export function SilButonu({ id, baslik }: { id: string; baslik: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-2">
      <span className="text-xs text-murekkep-700">
        &ldquo;{baslik}&rdquo; silinsin mi?
      </span>
      <button
        type="button"
        disabled={bekliyor}
        onClick={() => baslat(() => duyuruSil(id))}
